...
from Utils_Smart.vetor_utils_smart import criar_vetor_automatico,criar_vetor_manual,carregar_valores_arquivo,multiplica_por_valor,eleva_a_valor,reduzir_a_fracao,substituir_negativo,ordenar_valores,embaralhar_valores,adicionar,remover,remover_posicao,editar_posicao

from Utils_Smart.utils_smart import print_tela,number
...
def menor_maior_posicao(vetor):
    #MIN E MAX JÁ ME RETORNAM MENOR E MAIOR VALOR
    menor_valor = min(vetor)
    maior_valor = max(vetor)

    posicao_menor = vetor.index(menor_valor) + 1
    posicao_maior = vetor.index(maior_valor) + 1
...
def valores_positivos_quantidade(vetor):
    # LIST COMPREHENSION GERA UMA LISTA DE PRODUTOS QUE ATENDEM CONDIÇÕES DE UM LUGAR 
    positivos = [numero for numero in vetor if numero > 0]
    quantidade = len(positivos)

    print("\n Quantidade de números positivos = {}".format(quantidade))
...
def valores_negativos_quantidade(vetor):
    # LIST COMPREHENSION GERA UMA LISTA DE PRODUTOS QUE ATENDEM CONDIÇÕES DE UM LUGAR 
    negativos = [numero for numero in vetor if numero < 0]
    quantidade = len(negativos)

    print("\n Quantidade de números negativos = {}".format(quantidade))
